...
export default function Trending() {
  ...
  const fetchTrendingVideos = useCallback(async (pageNum = 1) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    ...
    if (videos[currentIndex]) {
      videos[currentIndex].scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }, [currentIndex]);
  ...
  const handleSave = async (id, type = 'video') => {
    ...
    if (type === 'news') {
      try {
        ...
        setVideos(videos => videos.map(video =>
          video._id === id
            ? { ...video, saved: response.saved, savedCount: response.savedCount }
            : video
        ));
      } catch (error) {
  ...
  return (
    <div className="trending-page" ref={containerRef} style={{ overflowY: 'auto', minHeight: '100vh', scrollSnapType: 'y mandatory' }}>
      ...
      {loading && videos.length === 0 ? (
        ...
        <div className="trending-videos-list" style={{ width: '100%', maxWidth: 420, margin: '0 auto' }}>
          {videos.map((video, idx) => (
            <div
              key={video._id}
              className="trending-video-snap"
              style={{ minHeight: 'calc(100vh - 80px)', scrollSnapAlign: 'start', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              <VideoCard
                video={video}
      ...
      {loading && videos.length > 0 && <div className="trending-loading">Loading more...</div>}
      {!hasMore && <div className="trending-empty">No more trending videos.</div>}
    </div>
  );
} 