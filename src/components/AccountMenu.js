...
const authenticatedOptions = [
  ...
  { key: 'communityposts', label: 'My Community Posts', icon: <PeopleIcon /> },
...
export default function AccountMenu({ open, onClose, onSelect, selected }) {
  ...
  const options = isAuthenticated ? authenticatedOptions : unauthenticatedOptions;

  return (
    <Modal open={open} onClose={onClose}>
      <Box className="account-menu-modal drawer-right">
        <button className="account-menu-close" onClick={onClose} title="Close"><CloseIcon /></button>
        ...
        {isAuthenticated && user && (
          <div className="account-menu-user-info">
            <div className="account-menu-avatar">
              <img 
                src={user.avatar || 'https://randomuser.me/api/portraits/men/99.jpg'} 
                alt={user.username} 
        ...
        <ul className="account-menu-list">
          {options.map(opt => (
            <li
              key={opt.key}
              className={`account-menu-item${selected === opt.key ? ' selected' : ''}`}
